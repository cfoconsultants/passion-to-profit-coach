...
export async function generateRoadmapPdf(r) {
  ...
  doc.setFontSize(9.5);
  setColor(ACCENT);
  doc.text('PASSION TO PROFIT COACH', PAGE_W / 2, y, { align: 'center' });
  y += 22;
  ...
  if (r.improvement_tips && r.improvement_tips.length > 0) {
    ...
    doc.setFontSize(10.5);
    setColor(ACCENT);
    doc.text('Ways to grow your score', MARGIN, y);
    y += 16;
    r.improvement_tips.forEach((t) => {
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(9.5);
      setColor(INK);
      ...
      doc.setFont('helvetica', 'normal');
      const wrapped = doc.splitTextToSize(t.tip, CONTENT_W - labelWidth);
